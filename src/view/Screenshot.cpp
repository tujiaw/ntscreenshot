...
// 背景透明色
static QColor BACKGROUND_COLOR(0, 0, 0, 160);
...
const std::shared_ptr<QPixmap>& ScreenshotWidget::getBackgroundScreen(void) {
	...
	// 制作暗色屏幕背景
	QPixmap pixmap(screenPixmap->width(), screenPixmap->height());
...
std::shared_ptr<QPixmap> ScreenshotWidget::getGlobalScreen(void) {
	if (!originScreen_) {
        ...
        if (!screen) {
            return std::make_shared<QPixmap>();
        }

		// 截取当前桌面，作为截屏的背景图
		QRect r = getDesktopRect();
        originScreen_ = std::make_shared<QPixmap>(screen->grabWindow(0, r.x(), r.y(), r.width(), r.height()));
	}
...
void ScreenshotWidget::updateMouse(void) {
	/// 禁用主窗口是为了排除getSmallestWindowFromCursor将其获取为选区，通过参数CWP_SKIPDISABLED
    QRect tmpRect;
    if (0) {
        ::EnableWindow((HWND)winId(), FALSE);
        defer(::EnableWindow((HWND)winId(), TRUE));
        Util::getSmallestWindowFromCursor(tmpRect);
    } else {
        /// 获取当前鼠标选中的窗口
        Util::getRectFromCurrentPoint((HWND)winId(), tmpRect);
    }

	QPoint p = mapFromGlobal(QPoint(tmpRect.x(), tmpRect.y()));
	QRect r(p.x(), p.y(), tmpRect.width(), tmpRect.height());
	if (getEsthesiaRect() != r) {
		setEsthesiaRect(r);
		update();
	}
}
...
void SelectedScreenWidget::scaledRect(int direction)
{
	...
	auto isColorSimilar = [&image](QRgb rgb, const QPoint &from, QPoint &to) -> bool {
        QColor src(rgb);
	...
	// left
    QRgb clr = image.pixel(left, top + (bottom - top) / 2);
	if (clr > 0) {
		for (int i = 1; i <= image.width(); i++) {
			int offset = i * direction;
            if (isColorSimilar(clr, QPoint(left + offset, top), QPoint(left + offset, bottom))) {
                offsetLeft = offset;
            } else {
                break;
            }
	...
	// right
    clr = image.pixel(right, top + (bottom - top) / 2);
	if (clr > 0) {
		for (int i = 1; i <= image.width(); i++) {
			int offset = -1 * i * direction;
            if (isColorSimilar(clr, QPoint(right + offset, top), QPoint(right + offset, bottom))) {
                offsetRight = offset;
            } else {
                break;
            }
	...
	// top
    clr = image.pixel(left + (right - left) / 2, top);
	if (clr > 0) {
		for (int i = 1; i <= image.height(); i++) {
			int offset = i * direction;
            if (isColorSimilar(clr, QPoint(left, top + offset), QPoint(right, top + offset))) {
                offsetTop = offset;
            } else {
                break;
            }
	...
	// bottom
    clr = image.pixel(left + (right - left) / 2, bottom);
	if (clr > 0) {
		for (int i = 1; i <= image.height(); i++) {
			int offset = -1 * i * direction;
            if (isColorSimilar(clr, QPoint(left, bottom + offset), QPoint(right, bottom + offset))) {
                offsetBottom = offset;
            } else {
                break;
            }
