/****************************************************************************
** Meta object code from reading C++ file 'DrawPanel.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.12.9)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../view/DrawPanel.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'DrawPanel.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.12.9. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_Drawer_t {
    QByteArrayData data[1];
    char stringdata0[7];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Drawer_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Drawer_t qt_meta_stringdata_Drawer = {
    {
QT_MOC_LITERAL(0, 0, 6) // "Drawer"

    },
    "Drawer"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Drawer[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       0,    0, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

       0        // eod
};

void Drawer::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    Q_UNUSED(_o);
    Q_UNUSED(_id);
    Q_UNUSED(_c);
    Q_UNUSED(_a);
}

QT_INIT_METAOBJECT const QMetaObject Drawer::staticMetaObject = { {
    &QObject::staticMetaObject,
    qt_meta_stringdata_Drawer.data,
    qt_meta_data_Drawer,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *Drawer::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Drawer::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_Drawer.stringdata0))
        return static_cast<void*>(this);
    return QObject::qt_metacast(_clname);
}

int Drawer::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    return _id;
}
struct qt_meta_stringdata_DrawPanel_t {
    QByteArrayData data[13];
    char stringdata0[151];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_DrawPanel_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_DrawPanel_t qt_meta_stringdata_DrawPanel = {
    {
QT_MOC_LITERAL(0, 0, 9), // "DrawPanel"
QT_MOC_LITERAL(1, 10, 10), // "sigSticker"
QT_MOC_LITERAL(2, 21, 0), // ""
QT_MOC_LITERAL(3, 22, 7), // "sigSave"
QT_MOC_LITERAL(4, 30, 11), // "sigFinished"
QT_MOC_LITERAL(5, 42, 18), // "onReferRectChanged"
QT_MOC_LITERAL(6, 61, 4), // "rect"
QT_MOC_LITERAL(7, 66, 17), // "onShapeBtnClicked"
QT_MOC_LITERAL(8, 84, 16), // "QAbstractButton*"
QT_MOC_LITERAL(9, 101, 17), // "onColorBtnClicked"
QT_MOC_LITERAL(10, 119, 16), // "onSettingChanged"
QT_MOC_LITERAL(11, 136, 8), // "fontSize"
QT_MOC_LITERAL(12, 145, 5) // "color"

    },
    "DrawPanel\0sigSticker\0\0sigSave\0sigFinished\0"
    "onReferRectChanged\0rect\0onShapeBtnClicked\0"
    "QAbstractButton*\0onColorBtnClicked\0"
    "onSettingChanged\0fontSize\0color"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_DrawPanel[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       7,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   49,    2, 0x06 /* Public */,
       3,    0,   50,    2, 0x06 /* Public */,
       4,    0,   51,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       5,    1,   52,    2, 0x0a /* Public */,
       7,    1,   55,    2, 0x0a /* Public */,
       9,    0,   58,    2, 0x0a /* Public */,
      10,    2,   59,    2, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void, QMetaType::QRect,    6,
    QMetaType::Void, 0x80000000 | 8,    2,
    QMetaType::Void,
    QMetaType::Void, QMetaType::Int, QMetaType::QColor,   11,   12,

       0        // eod
};

void DrawPanel::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<DrawPanel *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->sigSticker(); break;
        case 1: _t->sigSave(); break;
        case 2: _t->sigFinished(); break;
        case 3: _t->onReferRectChanged((*reinterpret_cast< const QRect(*)>(_a[1]))); break;
        case 4: _t->onShapeBtnClicked((*reinterpret_cast< QAbstractButton*(*)>(_a[1]))); break;
        case 5: _t->onColorBtnClicked(); break;
        case 6: _t->onSettingChanged((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< QColor(*)>(_a[2]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            using _t = void (DrawPanel::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&DrawPanel::sigSticker)) {
                *result = 0;
                return;
            }
        }
        {
            using _t = void (DrawPanel::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&DrawPanel::sigSave)) {
                *result = 1;
                return;
            }
        }
        {
            using _t = void (DrawPanel::*)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&DrawPanel::sigFinished)) {
                *result = 2;
                return;
            }
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject DrawPanel::staticMetaObject = { {
    &QWidget::staticMetaObject,
    qt_meta_stringdata_DrawPanel.data,
    qt_meta_data_DrawPanel,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *DrawPanel::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *DrawPanel::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_DrawPanel.stringdata0))
        return static_cast<void*>(this);
    return QWidget::qt_metacast(_clname);
}

int DrawPanel::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 7)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 7;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 7)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 7;
    }
    return _id;
}

// SIGNAL 0
void DrawPanel::sigSticker()
{
    QMetaObject::activate(this, &staticMetaObject, 0, nullptr);
}

// SIGNAL 1
void DrawPanel::sigSave()
{
    QMetaObject::activate(this, &staticMetaObject, 1, nullptr);
}

// SIGNAL 2
void DrawPanel::sigFinished()
{
    QMetaObject::activate(this, &staticMetaObject, 2, nullptr);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
