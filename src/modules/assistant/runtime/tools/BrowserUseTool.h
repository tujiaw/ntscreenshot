...
public:
    ...
    QString description() const override {
        return QStringLiteral(
            "操作右侧可见浏览器，保持同一登录会话。action: open(url)、read、click、fill、select、scroll(text为up/down)、back、wait_for、wait_user。"
            ...
            "click/fill/select 优先使用 target 语义定位（按钮文字、标签、placeholder、aria-label、名称或测试标识），只有无法唯一匹配时才使用最新 snapshot 和 element 编号。操作后自动返回新快照，失败含 snapshot 时直接使用，无需重复 read。"
            "动态页面先用 wait_for(target,timeoutMs) 等待目标出现，再执行操作；不要用重复 click 代替等待。"
    ...
    QJsonObject parameters() const override {
        return {{"type", "object"}, {"properties", QJsonObject{
            {"action", QJsonObject{{"type", "string"}, {"enum", QJsonArray{"open", "read", "click", "fill", "select", "scroll", "back", "wait_for", "wait_user"}}}},
            {"url", QJsonObject{{"type", "string"}}}, {"element", QJsonObject{{"type", "integer"}}}, {"target", QJsonObject{{"type", "string"}}},
            ...
            {"timeoutMs", QJsonObject{{"type","integer"},{"minimum",250},{"maximum",30000}}},
