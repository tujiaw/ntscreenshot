...
void OpenAIChat::postConversation()
{
    ...
    if (!toolDefinitions_.isEmpty()) {
        systemContent += QStringLiteral(
            ...
            "- `browser_use`：操作右侧可见浏览器。用户要求浏览器操作、登录后读取、点击或填表时使用此工具；整个任务保持在同一会话，不切换到后台工具。先 read 获取 snapshot 和元素编号，每步检查结果。\n"
            "- 仅当页面需要密码、验证码、扫码或身份认证时，才用 browser_use 的 wait_user 交给用户；不要索取用户的密码或验证码。普通浏览、搜索、点击、填表要自主完成，不要停下来让用户确认；控件不可操作时先重新 read 获取最新 snapshot，而不是交给用户。用户也可能同时在同一页面上手动操作，这不会打断你，继续你的任务即可；页面与你上次 read 的不一致时重新 read。\n"
